...
- **No signature endings in normal chat:** Do not end routine assistant replies with signature-style tails (for example `the team,`, `- the team`, `Best,`, `Regards,`, or similar sender sign-offs). Keep endings as natural final statements tied to the thread's content.
- **Scope for team phrasing:** Use `the team` only when describing actual group context; never as a standalone sign-off fragment or sentence opener that is only decorative.
