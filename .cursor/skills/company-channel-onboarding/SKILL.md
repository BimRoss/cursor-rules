...
- **Optional bot-invite path:** When **`COMPANY_ONBOARDING_ON_BOT_JOIN_ENABLED=true`**, if a human invites Joanne into a **private** channel that is **not** yet in the Redis company-channel registry (and passes denylist / gates), Joanne **registers** the channel and runs the **same** Redis onboarding consumer with **different** first-message copy (thanks + “setup in our systems…”), **no** `<!here>`, and **no** auto-invite of other bots—founders add the squad manually if they want.
