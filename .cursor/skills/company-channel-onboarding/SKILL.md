...
- After create, an **internal Redis queue** drives a Joanne **consumer** that posts **(1)** welcome to the channel name, company-workspace line, squad intro (`<!here>`), and a `-----` separator, then **(2)** “Tell me, *FirstName*, …” (plain first name(s) for human founders from Slack profile—**not** `<@U…>`), the three numbered path prompts, and an optional **manage your company** link to `/{channelId}` on the MakeACompany web origin—in the **new channel** (not in `#humans`). `<!here>` in **(1)** already notifies the room; the “Tell me” line does not need user @mentions. That is the end of Joanne’s scripted onboarding; **no** Tim auto-invite and **no** follow-up bot messages from those numbered choices.
